...
def _collate_fn(batch, size_list):
    ...
    global collate_idx
    collate_idx += 1
    print(f"Current size is {size} ", collate_idx)
...
if __name__ == '__main__':
    loader = create_loader(
        data_path=arg_config['rgb_data']['tr_data_path'], mode='train', get_length=False,
        size_list=arg_config['size_list']
    )
    
    for idx, train_data in enumerate(loader):
        train_inputs, train_masks, *train_other_data = train_data
        print(f"batch: {idx} ", train_inputs.size(), train_masks.size())
